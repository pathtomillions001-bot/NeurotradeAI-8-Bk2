...
// NeuroTrade embed: the builder is mounted under /bot/ (same origin as the
// trading platform) so the app's session cookie + localStorage are shared and
// the user is never asked to log in again. Set by the workspace build script.
const isEmbedded = process.env.NEXT_PUBLIC_DBOT_EMBEDDED === 'true';
const embedAssetPrefix = '/bot/';
...
export default defineConfig({
  ...
  source: {
    ...
    define: {
      'process.env': {
        ...
        // Embedded (NeuroTrade) mode: trims the builder to the workspace + run
        // panel, hides Deriv login/signup and routes account/socket setup through
        // the host app's own API instead of OAuth.
        NEXT_PUBLIC_DBOT_EMBEDDED: JSON.stringify(process.env.NEXT_PUBLIC_DBOT_EMBEDDED ?? ''),
  ...
  output: {
    assetPrefix: isStaticBuild ? '/bot/preview/' : isEmbedded ? embedAssetPrefix : '/',
    distPath: {
  ...
  server: {
    compress: true,
    // Same-origin embedding: any host the platform serves (Railway domain,
    // Replit preview, e2b preview) must be accepted, otherwise the dev server
    // returns 403 "Invalid Host header" inside the iframe.
    // Mounted under /bot so the dev server's own HTML emits /bot/-prefixed asset
    // URLs, matching the production assetPrefix. The trading platform proxies
    // /bot/* here unchanged, so the builder is same-origin with the host app in
    // dev exactly as it is in production.
    base: isEmbedded ? embedAssetPrefix.replace(/\/$/, '') : '/',
    host: '0.0.0.0',
    port: 4003,
    allowedHosts: ['*'],
  },
