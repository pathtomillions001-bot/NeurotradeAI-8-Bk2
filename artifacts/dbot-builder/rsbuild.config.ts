...
// NeuroTrade serves the embedded builder under its own base path (default /dbot/).
const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? (isStaticBuild ? '/bot/preview/' : '/');
  ...
  source: {
    ...
    define: {
      'process.env': {
        ...
        // Base path the embedded build is served under (router basename + assetPrefix).
