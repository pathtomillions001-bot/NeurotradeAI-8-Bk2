...
import { getHostSocketUrl } from '@/services/neurotrade-bridge';
...
export const getSocketURL = async (): Promise<string> => {
    ...
    // The host app already owns the session. Ask it for a single-use OTP URL for
    // the account it has selected (demo or real), instead of running Deriv OAuth
    // here. The bearer token never leaves the NeuroTrade API.
    if (isEmbeddedMode()) {
        try {
            const hostUrl = await getHostSocketUrl();
            if (hostUrl) return hostUrl;
        } catch (error) {
            console.error('[NeuroTrade] host socket URL unavailable:', error);
        }
        // No connected account (or the host refused): fall back to the public
        // server so the workspace still loads for building — trading will show
        // the host's "connect an account" state.
