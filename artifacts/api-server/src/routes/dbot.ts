/**
 * DBot routes — "scan → build a Deriv bot → run it inside the app".
 *
 * Mounted at /api/bots/dbot.
 *
 *   POST /overunder-turbo/build
 *     Turns an Over/Under Turbo scan lock into a Blockly XML strategy for the
 *     embedded Deriv bot builder (at /dbot). The generated bot trades the exact
 *     locked (market, normal, recovery) triple with the app's shared
 *     debt-driven recovery stake formula, TP/SL and a consecutive-loss breaker.
 *
 *   GET /session
 *     Returns THIS browser session's active Deriv account (login id + token)
 *     so the Bot Builder page can hand it to the embedded builder, which then
 *     talks straight to Deriv's WebSocket.
 */
...
router.get("/session", async (req, res): Promise<void> => {
  try {
    ...
    res.json({
      ...
      token: account.bearerToken ?? account.token,
      currency: account.currency,
      balance: account.balance ? Number(account.balance) : 10000,
