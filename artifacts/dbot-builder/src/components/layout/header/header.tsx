...
const AppHeader = observer(() => {
    ...
    const renderAccountSection = useCallback(
        (position: 'left' | 'right' = 'right') => {
            ...
            // Account identity comes from the host app (see
            // services/neurotrade-bridge.ts): the user connected once in
            // NeuroTrade. Deriv's own Log in / Sign up / Transfer / Logout
            // controls must not appear here — they would either start a second
            // login or navigate to Deriv's platform, both of which the embedding
            // exists to prevent. Only the read-only account chip is shown.
            if (isEmbeddedMode()) {
                if (position !== 'right' || !activeLoginid || is_account_regenerating) return null;
                return (
