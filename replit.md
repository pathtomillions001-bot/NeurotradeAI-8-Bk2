...
- `artifacts/api-server/src/lib/apex-analysis.ts` — 7th bot: the APEX stack (walk-forward replay of the entry rule, exact Markov-chain-imbedded ladder ruin, closed-form ladder depth + mean time to break, KT context model, Wald SPRT, anytime-valid confidence sequence, Page–Hinkley, stationarity, concordance, BH-FDR)
- `artifacts/api-server/src/lib/apex-timing.ts` — 7th bot: the entry-timing layer (momentum, Markov state preference, renewal clock, feed freshness, shot spacing, patience valve)
- `artifacts/api-server/src/lib/apex-engine.ts` — 7th bot: analyse once → lock the market → wait → fire, with a drift guard that never rotates
- `artifacts/api-server/src/lib/apex-analysis.test.ts` — 7th bot: the ladder formulas checked against Monte Carlo, the replay checked for look-ahead
- `artifacts/trading-platform/src/components/apex-console.tsx` — 7th bot console (contract + certainty bar, then the three gates shown separately)
...
- **The Apex bot prices its own entry rule (7th bot)**: a "high accuracy" bot built from stacked thresholds is untestable — nobody knows what the thresholds do until real money is on them. So `lib/apex-analysis.ts` takes the EXACT rule the engine fires on and replays it tick-by-tick over each market's own digit history using only information available at that tick (`replayEntryRule`, verified look-ahead-free by a prefix test). Markets are then ranked on the properties of THOSE shots — how many fired, how many won, and how the losses arrived — rather than on a statistic computed over the whole stream. The replay prices the conditional entry rule; the timing layer only defers entries, so shot ACCURACY transfers to the live bot while shot CADENCE is approximate (stated on the scan card, not hidden).
- **"No consecutive losses" is computed, not asserted (7th bot)**: the shared recovery ladder is debt-driven, so debt(k) = baseStake·(1+a)^(k−1) with a = (1+markup)/(payout−1) and it fails at the first k whose stake exceeds the cap — or whose debt reaches the stop loss. `ladderDepthLimit` solves both in closed form for k*, the number of consecutive losses the user's own stake/payout/markup/cap/stop-loss can absorb. The loss sequence is a 2-state Markov chain and P(a run deeper than k* occurs within N shots) is then EXACT by finite Markov chain imbedding (Fu & Koutras 1994) — an absorbing state at k*+1, iterated N times, no Monte Carlo. `expectedShotsToLadderBreak` gives the closed-form mean waiting time E[T_k] = [1 + r(1−q^(k−1))/(1−q)]/(r·q^(k−1)), which reduces to the classical (1−p^k)/((1−p)p^k) when r = q = p. Both are checked against Monte Carlo in `apex-analysis.test.ts`.
...
- **The market is locked and stays locked (7th bot)**: `LOCKED_SYMBOL` and `LOCKED_CONTRACT` are consts captured once at session start and there is no hunt mode, no rotation and no re-selection anywhere in `apex-engine.ts`. Because a lock cannot be corrected mid-session, a Page–Hinkley detector (mirrored for a FALL, past-only running mean, δ = 0.03, λ = 10) watches the locked market's realised win rate: when it fires the bot STOPS FIRING rather than moving market, and after 4 consecutive flags the session ends and asks for a fresh analysis.
...
- **The certainty bar is a user decision, not a hidden constant (7th bot)**: Elite / Strict (default) / Balanced in `APEX_CERTAINTY`. The previous bot of this kind hard-coded one severe bar and the practical result was a bot that never traded, which is indistinguishable from a broken one. Every gate prints itself when it blocks a market, and the scan names the structural headroom per contract rather than advising a re-scan.
...
- **Apex One-Shot Sniper** (`/bots` → BOT-APEX): one user-named contract (Over N / Under N / Matches / Even / Odd — never both sides), the AI analyses every digit market and LOCKS the best one, then waits for the market, the context and the tick to all agree. No rotation, no trade on deploy, and a certainty bar the user chooses.
