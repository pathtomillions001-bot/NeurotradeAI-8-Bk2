...
- `artifacts/api-server/src/lib/killshot-analysis.ts` — 7th bot: the seven-layer evidence stack (Wald SPRT, KT context model, anytime-valid confidence sequence, loss-pairing Markov, stationarity, multi-horizon concordance, BH-FDR)
- `artifacts/api-server/src/lib/killshot-timing.ts` — 7th bot: the separate execution-timing layer (momentum, renewal clock, feed freshness, shot spacing)
- `artifacts/api-server/src/lib/killshot-engine.ts` — 7th bot: hunt → wait → fire loop
- `artifacts/trading-platform/src/components/killshot-console.tsx` — 7th bot console (contract + hunt/lock, then evidence bar and entry bar)
...
- **The Kill-Shot bot separates WHAT from WHEN (7th bot)**: the user's CONTRACT is frozen for the session and is sovereign — hunt mode can never change it. The MARKET is not: `targetMode: "hunt"` (default) re-scores every digit market continuously and rotates to a challenger only when it wins by 8+ confidence points or the held market has been non-deployable for 3 passes, and never rotates while a recovery ladder is open. Firing is a second, independent decision: once the evidence stack clears, `lib/killshot-timing.ts` decides whether THIS tick is the entry (noise-scaled momentum at 1.25σ, the contract's own renewal clock, feed freshness, and ≥8 ticks since the last shot so each shot is an independent bet), with a patience valve that fires anyway after 45 ticks so a conclusive setup never rots.
- **The kill-shot bar must be reachable, not just severe**: H₁ is break-even + 2 ABSOLUTE points, never a relative fraction — a relative δ set H₁ at 97% on Over 0, a rate no digit stream reaches, so the SPRT could only abandon and the bot could never fire on ANY contract. Evidence is satisfied at one of two tiers: PRIME (the anytime-valid floor itself clears break-even) or STANDARD (the SPRT fired and the floor sits within a 9% relative shortfall, with the cross-market BH screen carrying the multiple-comparison risk). Loss pairing is gated on the one-sided 95% LOWER bound on ξ, not its point estimate, because ξ wanders either side of 1.0 on an independent stream from sampling noise alone. Every Deriv digit contract pays below its fair rate, so an unbiased stream is always ~1.2–1.7 points −EV — the scan reports that headroom per contract rather than advising a re-scan.
...
- **Kill-Shot Precision Sniper** (`/bots` → BOT-KILLSHOT): one user-named contract, markets hunted continuously, no trade on deploy — the bot waits for conclusive evidence AND a good entry tick.
