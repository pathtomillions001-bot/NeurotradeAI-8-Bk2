/**
 * Parity Forge routes — Even/Odd parity specialist with recovery-first intelligence.
 *
 * Mounted at /api/bots/parity-forge (see routes/bots.ts). The bot deploys ONLY
 * from its own console: the generic /:botId/* specialist endpoints refuse it,
 * and every start re-validates the side mode, market and the measured card the
 * scan produced. There is deliberately NO pace parameter — one mode, pacing
 * valve for normal, static bar for recovery.
 */
...
import {
  ...
  startSession,
  stopSession,
  type ParityForgeCandidate,
} from "../lib/parity-forge-engine";

const router = Router();

function parseSideMode(raw: unknown): ParityForgeSideMode {
  if (raw === "even") return "even";
  if (raw === "odd") return "odd";
  return "both";
}

function parseParams(raw: any): ParityForgeParams | null {
...
function parseCandidate(raw: any): ParityForgeCandidate | null {
  if (!raw || typeof raw !== "object") return null;
  ...
  return raw as ParityForgeCandidate;
}
...
router.get("/status", (req, res) => {
  ...
  if (owner && owner !== req.sessionId) {
    res.json({ ...status, running: false, sessionId: null, config: undefined, parityForgeDeployed: undefined, parityForgeWatch: undefined });
    return;
  }
  res.json(status);
});
...
router.post("/scan", async (req, res): Promise<void> => {
  try {
    const result = await scanForParityForge(req.sessionId);
    res.json(result);
  } catch (err) {
    logger.error({ err }, "Parity Forge scan failed");
...
router.post("/start", async (req, res): Promise<void> => {
  ...
  logger.info({ botId: PARITY_FORGE_BOT_ID, symbol: market.symbol, sideMode, marketMode }, "Parity Forge deployed");
  const status = getStatus();
