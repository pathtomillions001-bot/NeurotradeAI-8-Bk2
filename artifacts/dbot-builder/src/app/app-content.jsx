...
const AppContent = observer(() => {
    ...
    // Embedded in NeuroTrade: the host app owns support chat. The Deriv
    // LiveChat widget must not be injected into its UI.
    if (!isEmbeddedMode()) useLiveChat(livechat_client_information);
    ...
    // Embedded: adopt the host app's connected account (demo or real, exactly as
    // selected there) BEFORE anything reads `active_loginid`. This is what makes
    // the builder open already authorised, with no second login — the OTP URL for
    // that account is fetched from the host API per connection (see
    // src/services/neurotrade-bridge.ts).
    React.useEffect(() => {
        ...
        bootstrapHostSession()
            .then(session => {
                ...
                // Re-init so the socket handshakes with the host's account.
                import('@/external/bot-skeleton').then(({ api_base }) => {
                    api_base.init(true).catch(err => console.error('[NeuroTrade] api re-init failed:', err));
                });
            })
