 ...
 * OTP requests — rapid manual trades, manual + engine overlap, two accounts
...
export interface ContractResult {
  ...
  sellPrice: number;
  entrySpot: number;
  /** True if the broker has not returned a settlement record. */
  missing?: boolean;
...
//
// One manual trade therefore cost 2 sockets + 2 REST calls.
 ...
 *
 * The journal manager consumes this adapter's raw-socket-style event API,
 * while every request still uses the account's single paced connection.
 * Closing the adapter does not tear down a connection shared with trading.
 */
export interface PooledSocket {
  ...
  // The pool owns reconnection; caller cleanup must not tear it down.
  on(event: "error", cb: (err: Error) => void): void;
  on(event: "close", cb: () => void): void;
  /** Underlying transport dropped; the pool manages reconnection. */
...
export async function getPooledSocket(
  ...
  // detached caller would leave a listener on the shared connection forever,
  ...
  return {
    ...
    on(event: "message" | "open" | "error" | "close" | "dropped", cb: any) {
      if (event === "message") {
        ...
        // Subscribers can observe reconnections without closing the shared socket.
      ...
      // not forwarded; the pool owns transient connection failures.
    },
...
/** Normalize broker timestamps, which may arrive in seconds or milliseconds. */
...
/** Retry transient proposal failures only; never automatically repeat a buy. */
