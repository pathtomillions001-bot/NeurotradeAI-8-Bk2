...
export const BOT_CATALOG: BotDefinition[] = [
  ...
  },
  {
    ...
    description:
      "The contract plan is fixed — you set the risk, the AI does everything else. NORMAL shots fire Over 4 + Under 5 at the same tick with equal stakes: the pair is complementary, so exactly one leg always wins and it never double-loses. RECOVERY shots fire Over 5 + Under 4 with equal stakes: they win on every closed digit EXCEPT 4 and 5, and the entire analysis budget of the bot is spent keeping the closed digit out of {4,5} — at no cost. Three fused digit models (forgetting Dirichlet, order-1 and order-2 Markov, inverse-variance blend) estimate P(next digit ∈ {4,5} | context); hard vetoes (4/5 hot clusters, post-4/5 states, hot markets) can never be overridden, and the entry bar is the market's own percentile — self-referential, so selectivity adapts to the stream. Every market is measured out of sample and the ranking DECIDES locked vs switching — you do not choose the market or its mode.",
    edge: [
