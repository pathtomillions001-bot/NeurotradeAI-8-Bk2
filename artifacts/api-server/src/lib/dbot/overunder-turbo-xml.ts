 ...
 * a Blockly XML strategy for the Deriv Bot Builder.
 *
 * The generated bot trades EXACTLY like the in-app engine's locked mode:
 *
 *   · 1-tick Digit Over/Under contracts, back-to-back.
 ...
 *     clamped to [0.35, maxStake] (mirrors calculateBotRecoveryStake +
 ...
 *     a consecutive-loss run ≥ maxConsecutiveLosses (the circuit breaker).
 */
...
export interface TurboDbotSpec {
  /** Deriv symbol, e.g. "R_100" or "1HZ100V" — the locked market from the scan. */
  symbol: string;
...
export function validateTurboDbotSpec(spec: TurboDbotSpec): void {
  ...
  if (!/^[A-Za-z0-9_]+$/.test(spec.symbol)) {
    throw new TurboDbotSpecError(`symbol "${spec.symbol}" is not a valid symbol`);
  }
}
...
/** Expression: the exact recovery stake, clamped like applyRecoveryStakeLimits. */
